...
//	int angle = dangles[m_direction];
	GLvector pos = m_drive_position;
	int angle = 360 - (int)MathAngle (m_position.x, m_position.z, pos.x, pos.z);
	angle %= 360;
	int turn = (int)MathAngleDifference ((float)m_drive_angle, (float)angle);
	m_drive_angle += SIGN (turn);
	pos += glVector (0.5f, 0.0f, 0.5f);
		
	{	MakePrimitive hp(GL_QUADS);
		glTexCoord2f (0, 0);   
