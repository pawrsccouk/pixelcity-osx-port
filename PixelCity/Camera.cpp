...
//	IniVector("CameraAngle"   , &angle.x   , &angle.y   , &angle.z   );
//	IniVector("CameraPosition", &position.x, &position.y, &position.z);
	// with
	CameraReset();
}
...
{
	char buf[64]; buf[0] = '\0';
	sprintf(buf, "Camera Update: Movement = (%lf, %lf)", movement.x, movement.y);
	DebugLog(buf);	
...
void CameraTerm (void)		
{
  ...
  IniVectorSet("CameraAngle"   , angle.x   , angle.y   , angle.z   );
  IniVectorSet("CameraPosition", position.x, position.y, position.z);
}
